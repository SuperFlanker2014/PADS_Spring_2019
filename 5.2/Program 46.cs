		...
		static void Main(string[] args)
		{
			...
			foreach (var cmd in commands)
			{
				...
				else if (cmd == "X")
				{
					...
					else
					{
						...
						while (cur_i <= heap_max_index && (heap[cur_i] > heap[left_child_index] || heap[cur_i] > heap[right_child_index]))
						{
							...
							{
								var ind = heap[left_child_index] <= heap[right_child_index] ? left_child_index : right_child_index;
				...
				else if (cmd.StartsWith("D"))
				{
					...
					//D1.Start();
					var i = dict[element_old];// Array.IndexOf(heap, element_old);
					//D1.Stop();
